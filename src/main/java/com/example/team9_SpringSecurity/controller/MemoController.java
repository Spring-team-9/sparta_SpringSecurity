...
public class MemoController {
    ...
    // 글 좋아요 등록
    @PostMapping("/api/memos/{id}/like")
    public ResponseEntity<MessageDto> createMemoLike(@PathVariable Long id, HttpServletRequest request) {
        MessageDto messageDto = memoService.createMemoLike(id, request);
        return new ResponseEntity<>(messageDto, HttpStatus.OK);
    }

    // 글 좋아요 삭제
    @DeleteMapping("/api/memos/{id}/like")
    public ResponseEntity<MessageDto> DeleteMemoLike(@PathVariable Long id, HttpServletRequest request) {
        MessageDto messageDto = memoService.deleteMemoLike(id, request);
        return new ResponseEntity<>(messageDto, HttpStatus.OK);
    }

    // 댓글 좋아요 등록
    @PostMapping("/api/memos/{id}/{replyId}/like")
    public ResponseEntity<MessageDto> createReplyLike(@PathVariable Long id, @PathVariable Long replyId, HttpServletRequest request) {
        MessageDto messageDto = memoService.createReplyLike(id, replyId, request);
        return new ResponseEntity<>(messageDto, HttpStatus.OK);
    }

    // 댓글 좋아요 삭제
    @DeleteMapping("/api/memos/{id}/{replyId}/like")
    public ResponseEntity<MessageDto> DeleteMemoLike(@PathVariable Long id, @PathVariable Long replyId, HttpServletRequest request) {
        MessageDto messageDto = memoService.deleteMemoLike(id, replyId, request);
        return new ResponseEntity<>(messageDto, HttpStatus.OK);
    }
